...
def findspikes(
    ...
    spv = set(np.where(v > thresh)[0].tolist() ) # find all points above threshold
    sps = set(np.where(dv > 0.0)[0].tolist() ) #  and find points where slope is positive
    sp = list(sorted(set.intersection(spv, sps)))
  # intersection defines putative spikes
    if len(sp) == 0:
        return (st, spk)  # nothing detected
    sp = np.sort(sp)  # make sure all detected events are in order (sets is unordered)

    # normal operating mode is fixed voltage threshold
    # for this we need to just get the FIRST positive crossing,

    if mode == "schmitt":
        sthra = np.where(np.diff(sp) > mingap)
        ...
        for k in sthr:
            ...
            if len(st) == 0:
                st = np.append(st, x[1])
            elif np.min(np.fabs(s0 - st)) > mingap:
                st = np.append(st, x[1])
    
    elif mode == "peak":
        l_spv = sorted(list(spv))
        pkwidth = 1.0e-3  # in same units as dt  - usually sec
        kpkw = int(pkwidth / dt)
        z = (np.array(np.where(np.diff(l_spv) > 1)[0]) + 1).tolist()
        ...
        for k in z:
            ...
            if interpolate:
                ...
                speak = x[1]
            if len(st) == 0:
                st = np.append(st, speak)  # always save the first one
            elif np.min(np.fabs(speak - st)) > mingap:
                st = np.append(st, speak)
...
if __name__ == "__main__":
    ...
    if options.cb:  # test clements bekkers
        ...
        events[
            [50, 100, 250, 350, 475, 525, 900, 1500, 2800, 5000, 5200, 7000, 7500],
        ] = 1
        tau1 = 3
    ...
    if options.findspikes:  # test the findspikes routine
        ...
        v = np.zeros_like(t) - 60.0*1e-3
        spikeshape = [0., 10, 20, 30, 50, 65, 80, 90, 80, 70, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5, 0., -5, -5, 0]
        spikeshape = np.array(spikeshape)*1e-3
        for tx in np.arange(10e-3, 100e-3, 20e-3):
            itx = int(tx/dt)
            v[itx:itx+len(spikeshape)] += spikeshape
        
        sp, _ = findspikes(t, v, thresh=-20e-3, dt=dt, mode="peak", interpolate=False)
        print("findSpikes")
        print("sp: ", sp)
        f = PL.figure(1)
        PL.plot(t, v, "ro-")
        ...
        for k in si:
            ik = int(k)
            spk.append(v[ik]) # np.argmax(v[ik - 1 :ik + 1]) + ik)
        PL.plot(sp, spk, "bs")
        # PL.ylim((0, 25))
        PL.draw()
        PL.show()
        ...
        for k in range(0, len(y)):
            sp = getSpikes(
                t, y[k], 0, tpts, tdel=0, thresh=0, selection=None, interpolate=True
            )
            print("r: %d" % k, "sp: ", sp)
    ...
    if options.sinefit:
        ...
        for phi in np.arange(-2.0 * np.pi, 2.0 * np.pi, np.pi / 8.0):
            y = A * np.sin(2.0 * np.pi * t * F + phi) + normal(0.0, 0.5, len(t))
            (a, p) = sinefit(t, y, F)
