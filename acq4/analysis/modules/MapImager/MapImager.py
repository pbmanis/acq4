...
class MapImager(AnalysisModule):
    def __init__(self, host):
        ...
        self.dbIdentity = (
            "MapImager"  ## how we identify to the database; this determines which tables we own
        )
        ...
        self._elements_ = OrderedDict(
            [
                (
                    ...
                    {
                        "type": "ctrl",
                        "object": DatabaseQueryWidget(self.dataManager()),
                        "size": (300, 200),
                        "host": self,
                    },
                ),
                ...
                (
                    ...
                    {
                        "type": "ctrl",
                        "object": MapImagerColorMapper(
                            filePath=os.path.join(modPath, "colorMaps"), host=self
                        ),
                        "size": (200, 300),
                        "pos": ("right", "Database Query"),
                    },
                ),
                (
                    "Contour Plotter",
                    {
                        "type": "ctrl",
                        "object": ContourPlotter(host=self),
                        "pos": ("below", "Color Mapper"),
                    },
                ),
                (
                    "Canvas",
                ...
                (
                    ...
                    {
                        "type": "ctrl",
                        "object": SpatialCorrelator(),
                        "size": (100, 100),
                        "pos": ("bottom", "Map Convolver"),
                    },
                ),
    ...
    def saveMA(self, fileName=None):
        ...
        arr = MetaArray(
            [self.imgData[p] for p in self.imgData.dtype.names],
            info=[
                {"name": "vals", "cols": [{"name": p} for p in self.imgData.dtype.names]},
                {
                    "name": "xPos",
                    "units": "m",
                    "values": np.arange(self.imgData.shape[0]) * self.spacing + x,
                },
                {
                    "name": "yPos",
                    "units": "m",
                    "values": np.arange(self.imgData.shape[1]) * self.spacing + y,
                },
