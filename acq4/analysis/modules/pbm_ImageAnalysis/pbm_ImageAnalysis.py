# -*- coding: utf-8 -*-
"""
ImageAnalysis is an analysis module for ACQ4.
...
class pbm_ImageAnalysis(AnalysisModule):
    def __init__(self, host, flowchartDir=None, dbIdentity="ImageAnalysis"):
        ...
        self.imageInfo = []
        self.ignoreFirst = (
            1  # ImagePhys_ignoreFirst # note this is a number of images, not T/F
        )
        self.rectSelect = True  #
        self.tStart = (
            0.0  # baseline time start = applies to the image: ImagePhys_BaseStart
        )
        ...
        self.physLPFChanged = (
            False  # flag in case the physiology LPF changes (avoid recalculation)
        )
        #        self.physSign = 0.0  # ImagePhys_PhysSign (detection sign for events)
        self.physThresh = (
            -50.0
        )  # ImagePhys_PhysThresh (threshold in pA to detect events)
        self.physThreshLine = None
        self.ratioImages = (
            False  # only set true once a ratio (reference) image is loaded
        )
        self.ROIfig = None
        self.baseImages = []
        ...
        self.imageData = np.array(
            None
        )  # Image Data array, information about the data is in the dataState dictionary
        self.lastROITouched = []
    ...
    def _build_window(self):
        ...
        self._elements_ = OrderedDict(
            [
                ...
                (
                    ...
                    {
                        "type": "ctrl",
                        "object": self.ctrlImageFuncWidget,
                        "host": self,
                        "size": (150, 300),
                    },
                ),
                (
                    ...
                    {
                        "type": "ctrl",
                        "object": self.ctrlPhysFuncWidget,
                        "pos": ("above", "Analysis"),
                        "size": (150, 300),
                    },
                ),
                (
                    ...
                    {
                        "type": "ctrl",
                        "object": self.ctrlROIFuncWidget,
                        "pos": ("above", "Physiology"),
                        "size": (150, 300),
                    },
                ),
                (
        ...
        self.ROI_Plot.plotItem.vb.setXLink(
            "Phys Plot"
        )  # not sure - this seems to be at the wrong level in the window manager
        ...
        self.ctrlROIFunc.ImagePhys_RecalculateROIs.clicked.connect(
        ...
        self.ctrlROIFunc.ImagePhys_CorrTool_HPF.stateChanged.connect(
            self.refilterCurrentROI
        )  # corr tool is the checkbox
        self.ctrlROIFunc.ImagePhys_CorrTool_LPF.stateChanged.connect(
            self.refilterCurrentROI
        )
        self.ctrlROIFunc.ImagePhys_ImgHPF.editingFinished.connect(
            self.refilterCurrentROI
        )  # ImgHPF is the is the spinbox
        self.ctrlROIFunc.ImagePhys_ImgLPF.editingFinished.connect(
            self.refilterCurrentROI
        )

        # Physiology analysis buttons and controls
        self.ctrlPhysFunc.ImagePhys_DetectSpikes.clicked.connect(self.detectSpikes)
        self.ctrlPhysFunc.ImagePhys_PhysThresh.valueChanged.connect(
        ...
        self.ctrlPhysFunc.ImagePhys_PhysLPF.valueChanged.connect(
            self.physLPF_valueChanged
        )
        #
        # Imaging analysis buttons
        #
        self.ctrlImageFunc.IAFuncs_Distance.clicked.connect(self.ROIDistances)
        self.ctrlImageFunc.IAFuncs_DistanceStrength.clicked.connect(
            self.ROIDistStrength
        )
        self.ctrlImageFunc.IAFuncs_NetworkGraph.clicked.connect(self.NetworkGraph)
        self.ctrlImageFunc.IAFuncs_Analysis_AXCorr_Individual.clicked.connect(
            self.Analog_Xcorr_Individual
        )
        self.ctrlImageFunc.IAFuncs_Analysis_AXCorr.clicked.connect(self.Analog_Xcorr)
        self.ctrlImageFunc.IAFuncs_Analysis_UnbiasedXC.clicked.connect(
            self.Analog_Xcorr_unbiased
        )
        self.ctrlImageFunc.IAFuncs_DistanceStrengthPrint.clicked.connect(
            self.printDistStrength
        )
    ...
    def initDataState(self):
        ...
        self.backgroundPlot.clearPlots()
        self.IXC_Strength = []
        self.ROIDistanceMap = []
        self.tc_bleach = []
    ...
    def updateRectSelect(self):
        ...
        if self.rectSelect:
            self.ROI_Plot.plotItem.vb.setLeftButtonAction(
                mode="rect"
            )  # use the rubber band box instead
            self.physPlot.plotItem.vb.setLeftButtonAction(
                mode="rect"
            )  # use the rubber band box instead
        else:
            self.ROI_Plot.plotItem.vb.setLeftButtonAction(
                mode="pan"
            )  # use the standard pan mode instead
            self.physPlot.plotItem.vb.setLeftButtonAction(
                mode="pan"
            )  # use the standard pan modeinstead
    ...
    def changeView(self):
        ...
        if view == "Reference Image":
            self.imageView.setImage(
                np.mean(self.imageData[self.baseImages, :, :], axis=0)
            )
            self.viewFlag = True
    ...
    def updateAnalysis(self):
        ...
        if self.physLPFChanged:  # only update if the LPF filter has changed
            self.readPhysiology(
                self.currentDataDirectory
            )  # re-read in case LPF has changed
        c = 0
        if self.currentRoi is None:
            return
        for (
            img
        ) in self.normData:  # pull from all the normalized data arrays (in a list)
    ...
    def makePhysROIPlot(self, dh, dlh):
        ...
        if self.firstPlot is False:
            (self.MPLFig, self.MPPhysPlots) = PL.subplots(
                num="Physiology-Fluor comparison",
                nrows=self.MPRnrows,
                ncols=self.MPRncolumns,
                sharex=True,
                sharey=False,
            )
        ...
        for i in range(self.nROI):
            ...
            self.MPPhysPlots[r, c].plot(
                self.imageTimes[0:ndpt], (self.FData[i, :] - 1.0) * 100.0
            )
        self.plotCount += 1
        PL.draw()
        if self.plotCount >= self.nPhysPlots:
            PL.show()
            self.ctrl.ImagePhys_PhysROIPlot.setCheckState(
                False
            )  # turn off now - to properly sequence reload
            (d1, s1) = os.path.split(self.currentFileName)
            (d2, s2) = os.path.split(d1)
            (d3, s3) = os.path.split(s2)
            sfn = s3 + "-" + s2 + "-" + s1
            PL.savefig(
                "/Users/Experimenters/Desktop/ePhysPlots/%s.png" % (sfn),
                dpi=600,
                format="png",
            )
    ...
    def loadSingleFile(self, dh):
        ...
        Args:
            dh (FileHandle): DataManager File handle (acq4)

        Raises:
            Exception: If no valid imaging file found

        Returns:
            bool: True if read file ok, False if failed
        """
        ...
        if self.dataStruct == "flat":
            ...
            if dh.isFile(): # from a single file
                ...
                if (
                    ...
                    and str(Path(dh.name()).name).startswith("video_")
                    and "camera" in requestType
                ):
                    print("Found video")
                ...
                if (
                    ...
                    fhandle = dh[
                        "PMT.ma"
                    ]  # get data from PMT, as raw trace information
                    ...
                    decombInfo = dh.info()["protocol"]["analysis"]["Imaging"][
                        "children"
                    ]["decomb"]
                    ...
                    os.path.isfile(os.path.join(dh.name(), "imaging.ma"))
                    and "imaging" in requestType
                ):
            ...
            while len(images) > 0:
                imgs = images.pop(0)
                img = np.concatenate(
                    [i[np.newaxis, :minFrames, ...] for i in imgs], axis=0
                )
        ...
        self.updateThisROI(
            self.lastROITouched
        )  # and make sure plot reflects current ROI (not old data)
        return True
    ...
    def restoreDecomb(self):
        ...
        if self.PMTInfo is None:
            return
        self.ctrl.ImagePhys_PMT_decomb.setValue(
            1e6 * self.PMTInfo["decombInfo"]["value"]
        )
    ...
    def filterPMT(self, sdt, lpf):
        if self.ctrl.ImagePhys_PMT_LPF_check.isChecked():
            ...
            #            print sdt, lpf
            if 1.0 / sdt < lpf / 2.0:  # force nyquist happiness
                lpf = 0.5 / sdt
                print("reset lpf to ", lpf)
            filtdata = Utility.SignalFilter_LPFBessel(
                self.pmtData.asarray()[0], lpf, 1.0 / sdt, NPole=4, bidir=True
            )
            return filtdata
    ...
    def processPMT(self):
        ...
        if self.ctrl.ImagePhys_PMT_auto_check.isChecked():
            (decombed, lag) = self.rs.measureMirrorLag(
                pmt_d, transpose=True, maxShift=100
            )
            lag *= (
                sdt / 2.0
            )  # lag from measureMirrorLag is  expressed in pixels - convert to time.
        ...
        itx = self.rs.extractImage(
            self.pmtData.xvals("Time"), offset=lag, subpixel=subpixel
        )
        self.imageTimes = itx[:, 0, 0]
        self.scanTimes = itx[
            0, :, :
        ]  # use times from first scan; will adjust offset later
        self.prepareImages()
    ...
    def prepareImages(self):
        ...
        if self.imageType == "PMT":
            self.imageView.imageItem.setTransform(
                pg.QtGui.QTransform.fromScale(
                    1.0,
                    abs(
                        (self.rs.width / self.rs.height)
                        / (
                            float(self.imageData.shape[1])
                            / float(self.imageData.shape[2])
                        ),
                        1.0,
                    ),
                ),
    ...
    def computeBackground(self):
        bkstd_threshold = 0.025
        bkstd = self.rawData.std(axis=0)
        bkmn = self.rawData.mean(axis=0)
        # print(self.rawData.shape)
        # print(bkstd.shape, bkmn.shape)
        bk_cv = bkstd / bkmn
        # bkstd[(bkstd/bkmn) < 0.025] = 0
        bkcv_mask = np.broadcast_to(bk_cv, self.rawData.shape)
        raw_masked = np.ma.masked_where(
            condition=(bkcv_mask < bkstd_threshold), a=self.rawData, copy=True
        )
        # print('mask shape: ', bkcv_mask.shape)
        # print('raw_masked shape: ', raw_masked.shape)
        self.background = np.ma.compressed(np.ma.mean(raw_masked, axis=(1, 2)))
        # print("background shape: ", self.background.shape)
        self.backgroundmean = np.mean(self.background, axis=0)
        self.backgroundPlot.plot(np.arange(len(self.background)), self.background, "y-")
    ...
    def spectrumCalc(self, bandpass:Union[List, bool]=False, sigma:Union[float, bool]=False):
        """
    ...
    def getImageScaling(self):
        ...
        if "pixelSize" in self.imageInfo[3]:
            ...
            print(
                "Old File without full scaling information on image, setting to defaults of pixels."
            )
            ...
            region = [
                ...
                sh[1],
                sh[2],
            ]  # make region from image data directly [x0,y0,y1,y1]
    ...
    def getFileInfo(self):
        ...
        imt = MetaArray(
            file=dh.name()
        )  # , subset=(slice(block_pos,block_pos+block_size),slice(None), slice(None)))
        sh = imt.shape
        info = imt.infoCopy()
        self.downSample = int(self.ctrl.ImagePhys_Downsample.currentText())
        if self.downSample <= 0:
    ...
    def tryDownSample(self, dh):
        ...
        if imt is None:
            raise HelpfulException(
                "Failed to read file %s in tryDownSample" % dh.name(), msgType="status"
            )
        sh = imt.shape
        info = imt.infoCopy()
        ...
        print(
            "Reducing from %d frames to %d frames, downsample = %d "
            % (sh[0], outframes, self.downSample)
        )
        ...
        with pg.ProgressDialog("Downsampling", 0, outframes) as dlg:
            ...
            for bb in range(nbigblocks):
                ...
                for fr in range(nframesperblock):
                    ...
                    if avgflag:
                        imt_out[tfr] = np.mean(
                            img[block_pos : (block_pos + self.downSample)], axis=0
                        )
    ...
    def readPhysiology(self, dh=None):
        ...
        if self.dataModel.getClampMode(data) == "IC":
            ...
            self.physData = (
                self.physData * 1e12
            )  # convert to pA, best for on-cell patches
            units = "pA"
        info1 = data.infoCopy()
        self.samplefreq = info1[2]["DAQ"]["primary"]["rate"]
        if (
            self.physLPF >= 250.0 and self.physLPF < 0.5 * self.samplefreq
        ):  # respect Nyquist, just minimally
            self.physData = Utility.SignalFilter_LPFBessel(
                self.physData, self.physLPF, self.samplefreq, NPole=8
            )
        ...
        self.physPlot.plot(
            tdat, self.physData[::decimate_factor], pen=pg.mkPen("w")
    ...
    def Baseline1(self, roi=None):
        ...
        for roi in lrois:
            ...
            pts = np.where(
                np.abs(dds[roi.ID]) < thr * stdev
            )  # get subset of points to fit
            ...
            #            dm = np.mean(d[0:10])
            bl = Utility.savitzky_golay(
                d / bd, kernel=self.FilterKernel, order=self.FilterOrder
            )
            self.BFData[roi.ID] = bl
        return self.BFData
    ...
    def SignalLPF(self, roi):
        ...
        if samplefreq / 2.0 < LPF:
            self.showdialog(
                f"LPF {LPF:.1f} Hz is above Nyquist limit of {samplefreq/2.0:.1f} Hz\nStopping."
            )
            return None
    ...
    def showPhysTrigger(self):
        ...
        if self.physThreshLine is None:
            self.physThreshLine = self.physPlot.plot(
                x=np.array([self.tdat[0], self.tdat[-1]]),
                y=np.array([thr, thr]),
                pen=pg.mkPen("r"),
                clear=False,
            )
        else:
            self.physThreshLine.setData(
                x=np.array([self.tdat[0], self.tdat[-1]]), y=np.array([thr, thr])
            )
    ...
    def detectSpikes(self, burstMark=None):
        ...
        if thr < 0:
            ...
            ysign = 1.0
        (sptimes, sppts) = Utility.findspikes(
            self.tdat,
            ysign * self.physData,
            np.abs(thr) * spikescale,
            t0=None,
            t1=None,
        ...
        self.makeSpikePointers(
            spikes=(sptimes, yspmarks),
            spikespk=(sptimes, self.physData[sppts]),
    ...
    def makeSpikePointers(self, spikes=None, spikespk=None, bursts=None):
        ...
        if spikes is not None and len(spikes[0]) > 0:
            if self.spikesFound is None:
                self.spikesFound = pg.ScatterPlotItem(
                    size=6,
                    pen=pg.mkPen("g"),
                ...
                self.spikesFound.setPoints(
                    x=spikes[0], y=spikes[1] * np.ones(len(spikes[0]))
                )

        if spikespk is not None and len(spikespk[0]) > 0:
            if self.spikesFoundpk is None:
                self.spikesFoundpk = pg.ScatterPlotItem(
                    size=4,
                    pen=pg.mkPen("r"),
                    brush=pg.mkBrush(0, 255, 0, 200),
                    symbol="o",
                ...
                self.spikesFoundpk.setPoints(
                    x=spikespk[0], y=spikespk[1] * np.ones(len(spikespk[0]))
                )

        if bursts is not None and len(bursts[0]) > 0:
            if self.burstsFound is None:
                self.burstsFound = pg.ScatterPlotItem(
                    size=7,
                    pen=pg.mkPen("y"),
            ...
            if self.withinBurstsFound is None:
                self.withinBurstsFound = pg.ScatterPlotItem(
                    size=7,
                    pen=pg.mkPen("b"),
            ...
            self.burstsFound.setPoints(
                x=onsetSpikes, y=[bursts[1] for x in range(len(onsetSpikes))]
            )
            self.withinBurstsFound.setPoints(
                x=burstSpikes, y=[bursts[2] for x in range(len(burstSpikes))]
            )
    ...
    def computeBTA(self, singleSpike=False):
        ...
        if (
            not singleSpike
        ):  # normal processing is to do bursts, using first spike of burst
            if self.burstTimes == []:
                bList = self.defineSpikeBursts()
                self.burstTimes = bList
            onsetSpikes = []
        ...
        for roi in range(0, self.nROI):
            ...
            for onSp in onsetSpikes:
                (x, y) = Utility.clipdata(
                    self.FData[roi], self.imageTimes, onSp - 0.1, onSp + 0.5
                )
                avCaF[roi][i] = y
        ...
        for roi in range(0, self.nROI):
            ...
            for i in range(0, len(onsetSpikes)):
                ...
                self.MPL_plots[roi][0].plot(
                    avCaT[roi][i], avCaF[roi][i] * 100.0, color="k", linestyle="-"
                )
                f_int = scipy.interpolate.interp1d(
                    avCaT[roi][i], avCaF[roi][i] * 100.0, bounds_error=False
                )
            ...
            self.MPL_plots[roi][1].errorbar(
                tbase, ave[roi] * 100.0, yerr=std[roi] * 100.0, color="r"
            )
        ...
        for roi in range(0, self.nROI):
            self.MPL_plots[roi][1].set_ylim((axmin * 100.0, axmax * 100.0))
            self.MPL_plots[roi][0].set_ylim((CaAmin * 100.0, CaAmax * 100.0))
    ...
    def defineSpikeBursts(self):
        ...
        for i in burstOnsetCandidates:
            ...
            for j in range(i, len(self.SpikeTimes) - 1):
                if (
                    isis[j] <= maxInterval
                ):  # if interspike interval is long, we terminate
                    tempWithinBurst.append(
                        j + 1
                    )  # keep track of spikes that are "within" a burst
                else:  # if isi is too long, terminate burst
                    break
            if (
                len(tempWithinBurst) >= (minNspikes - 1) and i not in burstOnsetList
            ):  # note, tempWithinBurst does not include the first spike.
    ...
    def ROIDistStrength(self):
        ...
        if self.use_MPL:
            ...
            (self.MPLFig, self.MPL_plots) = PL.subplots(
                num="Image Analysis", nrows=1, ncols=1, sharex=True, sharey=True
            )
            self.MPLFig.suptitle("Analog XCorr: %s" % self.currentFileName, fontsize=11)
            self.MPL_plots.scatter(
                self.ROIDistanceMap, self.IXC_Strength, s=15, color="tomato"
            )
            ...
            self.floatingDistWin.setWindowTitle(
                "ROI Distance Strength: %s" % self.currentFileName
            )
            self.floatingDistWin.layout.clear()
            self.floatingDistWin.layout.setWindowTitle("New Title?")
            s1 = pg.ScatterPlotItem(
                size=7, pen=pg.mkPen(None), brush=pg.mkBrush(255, 0, 0, 255)
            )
        ...
        if len(x) > 0:  # not an empty array passed
            ...
            if x_min != x_max:  # esp. not both x_min,x_max equal to zero
                ...
                if x_min - buffer > 0:  # both (x_min-buffer),(x_max+buffer) > 0
                    x_min = round(
                        x_min - buffer, -int((np.floor(np.log10(buffer) - 1)))
                    )
                    ...
                    x_max = round(
                        x_max + buffer, -int((np.floor(np.log10(buffer) - 1)))
                    )
                else:  # (x_min-buffer </= 0)and(x_max+buffer >/= 0)
                    try:
                        x_min = round(
                            x_min - buffer, -int((np.ceil(np.log10(buffer) - 1)))
                        )
                    except OverflowError:  # buffer == 0
                        x_min = 0
                    try:
                        x_max = round(
                            x_max + buffer, -int((np.ceil(np.log10(buffer) - 1)))
                        )
                    except OverflowError:  # buffer == 0
                        x_max = 0
            else:
    ...
    def printDistStrength(self):
        ...
        for i in range(0, sh[0]):
            for j in range(i + 1, sh[1]):
                print(
                    "%d\t%d\t%8.0f\t%6.3f"
                    % (i, j, self.ROIDistanceMap[i, j], self.IXC_Strength[i, j])
                )
        print("-------------------------------\n")
    ...
    def NetworkGraph(self):
        ...
        if self.use_MPL:
            (self.MPLFig, self.MPL_plots) = PL.subplots(
                num="Network Graph", nrows=1, ncols=1, sharex=True, sharey=True
            )
            self.MPLFig.suptitle(
                "Network Graph: %s" % self.currentFileName, fontsize=11
            )
            yFlip_flag = False
        else:
            self.floatingDistWin = pyqtgrwindow(title="Network Graph")
            self.floatingDistWin.setWindowTitle(
                "Network Graph: %s" % self.currentFileName
            )
            self.floatingDistWin.layout.clear()
        ...
        for i in range(0, nd):
            wpos1 = [
                self.AllRois[i].pos().x(),
                self.AllRois[i].pos().y(),
                self.AllRois[i].boundingRect().width(),
                self.AllRois[i].boundingRect().height(),
            ]
            ...
            for j in range(i + 1, nd):
                wpos2 = [
                    self.AllRois[j].pos().x(),
                    self.AllRois[j].pos().y(),
                    self.AllRois[j].boundingRect().width(),
                    self.AllRois[j].boundingRect().height(),
                ]
                ...
                if np.abs(self.IXC_Strength[i, j]) < threshold:
                    ...
                    lw = (
                        maxline
                        * (abs(self.IXC_Strength[i, j]) - threshold)
                        / (maxStr - threshold)
                        + minline
                    )
        ...
        if self.use_MPL:
            ...
            plt.setXRange(0.0, sx)
            plt.setYRange(0.0, sy)
    ...
    def deleteLastTouchedROI(self):
        ...
        for roi in self.AllRois:  # navigate the list one more time
            if id == roi.ID:
                self.updateThisROI(
                    roi
                )  # display the next chosen ROI in the box below the image
        # now update the overall ROI plot
        self.plotdata(yMinorTicks=0, yMajorTicks=3, yLabel="F0<sub>ROI %d</sub>")
    ...
    def addOneROI(self, pos=(0, 0), hw=None):
        ...
        roi.addRotateHandle(
            pos=(0, 0), center=(0.5, 0.5)
        )  # handle at left top, rotation about center
        ...
        roi.sigRegionChanged.connect(
            self.updateThisROI
        )  # if data region changes, update the information
        roi.sigHoverEvent.connect(
            self.showThisROI
        )  # a hover just causes the display below to show what is hre already.
        return roi
    ...
    def updateThisROI(self, roi, livePlot=True):
        ...
        if roi in self.AllRois:
            tr = roi.getArrayRegion(
                data=np.dstack(self.imageData),
                img=self.imageView.imageItem,
                axes=[0, 1],
            )
    ...
    def scannerTimes(self, roi):
        ...
        """
        tr = roi.getArrayRegion(
            data=self.scanTimes, img=self.imageView.imageItem, axes=[2]
        )
        tr = (
            tr.mean()
        )  # axis=1).mean(axis=0)  # compute average over the ROI against time
    ...
    def showThisROI(self, roi, livePlot=True):
        ...
        if roi in self.AllRois:
            if livePlot is True:
                if self.imageType == "camera":
                    ...
                    raise ValueError(
                        "Image type for time array not known: %s", self.imageType
                    )
                try:
                    roi.plot.setData(
                        times,
                        self.BFData[roi.ID],
                        pen=pg.mkPen(np.append(roi.color[0:3], 255), width=1.0),
                    )  # , pen=pg.mkPen(roi.color), clear=True)
                except:
                    roi.plot = self.ROI_Plot.plot(
                        times,
                        self.BFData[roi.ID],
                        pen=pg.mkPen(np.append(roi.color[0:3], 255), width=1.0),
                        clear=False,
                    )  # pg.mkPen('r'), clear=True)
                c = roi.color[:3]  # np.append(roi.color[0:3], 255)
        ...
        for otherroi in self.AllRois:
            if otherroi != roi:
                c = otherroi.color[:3]
                otherroi.plot.setPen(pg.mkPen(color=c, width=1.0, alpha=0.3))
    ...
    def refilterCurrentROI(self):
        ...
        if roi in self.AllRois:
            if self.applyROIFilters(roi):
                self.ROI_Plot.plot(
                    self.imageTimes, self.BFData[roi.ID], pen=pg.mkPen("r"), clear=True
                )
    ...
    def applyROIFilters(self, rois):
        ...
        #            self.BFData = []
        for roi in rois:
            self.BFData = self.insertFData(
                self.BFData, self.FData[roi.ID], roi
            )  # replace current data with raw data
            if self.ctrl.ImagePhys_CorrTool_BL1.isChecked():
                bl = self.Baseline1(roi)
                self.BFData = self.insertFData(self.BFData, bl, roi)
            if (
                self.ctrlROIFunc.ImagePhys_CorrTool_LPF.isChecked()
                and self.ctrlROIFunc.ImagePhys_CorrTool_HPF.isChecked()
            ):
    ...
    def __measDither(self, ditherMode, ourWidget):
        ...
        trDither = ourWidget.getArrayRegion(
            self.normData[0], self.imageItem, axes=(1, 2)
        )
        trDither = trDither.mean(axis=2).mean(
            axis=1
        )  # compute average over the ROI against time
    ...
    def ROIDistances(self):
        ...
        nd = len(self.AllRois)
        self.ROIDistanceMap = np.empty(
            (nd, nd)
        )  # could go sparse, but this is simple...
    ...
    def saveROI(self, fileName=None):
        ...
        if fileName is None or fileName is False:
            fileName = Qt.QFileDialog.getSaveFileName(
                None, "Save ROI as csv file", "", self.tr("CSV Files (*.csv)")
            )
            if not fileName:
                return
    ...
    def restoreROI(self, fileName=None):
        ...
        if fileName is False or fileName is None:
            fileName = Qt.QFileDialog.getOpenFileName(
                None, "Retrieve ROI data", "", "ROIs (*.roi)"
            )
        if fileName:
            fd = open(fileName, "r")
            for line in fd:
    ...
    def makeROIDataFigure(self, clear=True, gcolor="k"):
        self.checkMPL()
        (self.MPLFig, self.MPL_plots) = PL.subplots(
            num="ROI Data", nrows=self.nROI, ncols=1, sharex=True, sharey=True
        )
        ...
        for i in range(self.nROI):
            self.MPL_plots[i].plot(
                self.imageTimes[0:ndpt], self.FData[i, :], color=gcolor
            )
    ...
    def stackOp_normmax(self):  # normalized maximum
        ...
        levels = [8.0, 16.0, 256.0, 4096.0, 65536.0]
        ...
        if len(id_shape) == 4:
            ...
            id = np.floor(
                (levels[levindex] / amaxd)
                * (self.imageData[:, plane, :, :].astype("float32") - amind)
            )
        elif len(id_shape) == 3:
            ...
            id = np.floor(
                (levels[levindex] / amaxd)
                * (self.imageData[:, :, :].astype("float32") - amind)
            )
    ...
    def stackOp_restore(self):
        ...
        self.paintImage(
            updateTools=True, focus=True
        )  # return to the original imagedata
    ...
    def unbleachImage(self):
        ...
        if bleachmode == "exp2":
            ...
            (fpar, xf, yf, names) = Fits.FitRegion(
                ...
                np.amax(fitx),
                fitFunc="exp2",
                fitPars=[0.9, 0.5, endT / 5.0, 0.5, endT / 2.0],
                plotInstance=None,
            )
            ...
            tau2 = fpar[0][4]
            self.tc_bleach = (
                DC + A0 * np.exp(-fitx / tau1) + A1 * np.exp(-fitx / tau2)
        ...
        if bleachmode == "2DPoly":
            ...
            for k in range(0, imshape[0]):
                z, u, r, s = np.linalg.lstsq(
                    A, self.imageData[k, :, :].reshape(imshape[1] * imshape[2], 1)
                )
                if k == 0:
                    print(z)
        ...
        for k in range(0, len(self.imageData)):
            ...
            if (
                bleachmode == "2DPoly"
            ):  # whole field correction, not just linear with time
                # print np.amax(np.amax(scaled_blimg, 0), 0)*tc_bleach[k], self.tc_offset[k]
                self.imageData[k, :, :] = (
                    self.imageData[k, :, :] - self.tc_offset[k]
                ) / (scaled_blimg * self.tc_bleach[k])
            else:
                self.imageData[k, :, :] = self.imageData[k, :, :] / (
                    self.tc_bleach[k] / mFluor
                )
        ...
        ndl = len(tc_bleach)
        self.backgroundPlot.plot(
            y=tc_bleach, x=self.imageTimes[0:ndl], pen=pg.mkPen("r"), clear=True
        )
        # self.backgroundPlot.plot(y=self.tc_bleach, x=self.imageTimes[0:ndl], clear=False, pen=pg.mkPen('b'))
        self.backgroundPlot.plot(
            y=b_corr, x=self.imageTimes[0:ndl], clear=False, pen=pg.mkPen("g")
        )
    ...
    def ProperOpen(self, img, block_size):
        return self.pOpen(
            self.pClose(self.pOpen(img, block_size), block_size), block_size
        )
    ...
    def findROIs(self):
        ...
        diag = np.hypot(
            dr, dr
        )  # note we only accept ROIs that are more than this distance apart - nonoverlapping
        ...
        thrlist = np.arange(
            thr_low, thr_high * 1.2, 0.05
        )  # start at lowest and work up
        import matplotlib.colors as mc

        thrcols = list(
            mc.cnames.keys()
        )  # ['r', 'orange', 'y', 'g', 'teal', 'c', 'b', 'violet', 'gray', '']
        # find countours for each threshold level
        for t in thrlist:
            thr = (maxt - meant) * t
            imctr = reconst2.copy()  # cv2 method may modify input argument
            retval, ci = cv2.threshold(
                imctr.astype("uint8"), thr, maxt, cv2.THRESH_BINARY
            )
            contours, heirarchy = cv2.findContours(
                ci, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            oth = []
            m = []
            pols[t] = []
            for cnt in contours:
                ...
                if area > 2.0 and area < 400:
                    ...
                    cnt = np.append(
                        cnt, cnt[0]
                    )  # add the first point to the array to make sure it is closed
                    oth.append([cnt, True])
            pols[t] = oth
        ...
        with pg.ProgressDialog("Searching for ROIs ...", 0, 100) as dlg:
            for i in range(
                len(thrlist) - 1
            ):  # work through all thresholds, starting at the bottom
                t = thrlist[i]
                ...
                for k1, s1 in enumerate(pols[t]):  # for each region at this threshold
                    ...
                    if dlg.wasCanceled():
                        raise HelpfulException(
                            "The search for ROIs was canceled by the user.",
                            msgType="status",
                        )
                    poly_low = np.array(
                        [s1[0].reshape(-1, 2)]
                    )  # this is needed for cv2.moments to take tha argument.
                    ...
                    cm_low = (
                        m["m10"] / m["m00"],
                        m["m01"] / m["m00"],
                    )  # compute center of mass of this point
                    for k2, s2 in enumerate(
                        pols[t2]
                    ):  # for each region identified at the next theshold level:
                        poly_high = np.array([s2[0].reshape(-1, 2)])
                        m_high = cv2.moments(poly_high)
                        cm_high = (
                            m_high["m10"] / m_high["m00"],
                            m_high["m01"] / m_high["m00"],
                        )  # compute center of mass of this point
                        test = cv2.pointPolygonTest(
                            poly_low, cm_high, False
                        )  # is that center of mass
                        if (
                            test >= 0
                        ):  # a higher threshold  center is definitely INSIDE the polygon of the lower threshold
                            oneabove = (
                                True  # we just need to find one - there could be more
                            )
                            break
                    if (
                        oneabove is False
                    ):  # no CM's were found above us, so save this value
                        finalregions[
                            nregs
                        ] = cm_low  # Accepte this polygon at this threshold as a candidate.
                        regthresh[nregs] = t
                        nregs += 1
        ...
        for i, p in enumerate(finalregions.keys()):
            if p in excluded:  #  or p in candidates_n:
                continue
            set_close = tree.query(
                fp[i], k=100, distance_upper_bound=diag
            )  # find all pairs that are close together
            neighbors = []
            ...
            if len(neighbors) == 1:  # we are our only neighbor
                candidates[p] = (
                    finalregions[p],
                    regthresh[p],
                )  # no decision to make, this one is isolated
                excluded.append(p)
                continue
            k = int(np.argmax(allth))  # find the one with the highest signal
            candidates[p] = (
                finalregions[neighbors[k]],
                allth[k],
            )  # candidates will have only the keys that are picked.
            for n in neighbors:
        ...
        for i, p in enumerate(nc.keys()):
            if p in excluded:
                continue
            set_close = tree.query(
                cp[i], k=10, distance_upper_bound=diag
            )  # find all pairs that are close together
            allth = []  # get the thresholds for all the neighbors
            neighbors = []
            for j, p1 in enumerate(set_close):
                ...
                neighbors.append(
                    p2
                )  # build a list of local friends, mapped to main list
    ...
    def slowFilterImage(self):
        ...
        if self.dataState["bleachCorrection"] is False:
            ...
            self.imageData = (
                self.rawData.copy()
            )  # just copy over without a correction        print 'Normalizing'
        if (
            self.dataState["Normalized"] is True
            and self.dataState["bleachCorrection"] is True
        ):
        ...
        smi = scipy.ndimage.filters.uniform_filter1d(
            self.imageData, axis=0, size=n_targetSmooth
        )
        smd = scipy.ndimage.filters.uniform_filter1d(
            self.imageData, axis=0, size=n_subSmooth
        )
        self.imageData = (
            smi[n_delay : sh[0], :, :] - smd[0 : sh[0] - n_delay + 1, :, :]
        )  # shifted subtraction, reduces data set by the time involved
    ...
    def normalizeImage(self):
        ...
        for i in range(len(self.imageData)):
            self.imageData[i, :, :] = (
                1.0 + (self.imageData[i, :, :] - meanimage) / meanimage
            )
        #        imstd = np.std(self.imageData, axis=0)
        #        imstd = scipy.ndimage.gaussian_filter(imstd, sigma=0.002)
        #        isize = 1
    ...
    def smoothImage(self):
        self.imageData = scipy.ndimage.filters.gaussian_filter(
            self.imageData, (3, 3, 3)
        )
        self.paintImage()
    ...
    def Analog_Xcorr(self, FData=None):
        ...
        if not self.use_MPL:
            ...
            self.floatingWindow.setWindowTitle(
                "Average XCorr: %s" % self.currentFileName
            )
            ...
            p.plot(
                [np.min(self.lags), np.max(self.lags)],
                [0, 0],
                pen=pg.mkPen(
                    {
                        "color": "#ff0",
            ...
            self.checkMPL()
            (self.MPLFig, self.MPL_plots) = PL.subplots(
                num="Average XCorr", nrows=1, ncols=1, sharex=True, sharey=True
            )
            self.MPLFig.suptitle(
                "Average XCorr: %s" % self.currentFileName, fontsize=11
            )
            self.MPL_plots.plot(self.lags, self.xcorr)
            self.MPL_plots.plot(self.lags, np.zeros(self.lags.shape), color="k")
            self.MPL_plots.plot(
                [0, 0], [-0.5, 1.0], color="k", linewidth=0.33, alpha=0.7
            )
    ...
    def Analog_Xcorr_Individual(self, FData=None, plottype="traces"):
        ...
        if not self.use_MPL:
            ...
            for xtrace1 in range(0, nROI - 1):
                for xtrace2 in range(xtrace1 + 1, nROI):
                    self.IXC_plots[xtrace] = self.floatingWindow.layout.addPlot(
                        xtrace1, xtrace2
                    )
                    if xtrace > 0:
            ...
            self.checkMPL()
            if plottype == "traces":
                (self.MPLFig, self.IXC_plots) = PL.subplots(
                    num="Individual ROI Cross Correlations",
                    nrows=self.nROI - 1,
                    ncols=self.nROI - 1,
                    sharex=True,
                    sharey=True,
                ...
                for xtrace2 in range(xtrace1 + 1, nROI):
                    ...
                    if plottype == "traces":
                        if not self.use_MPL:  # pyqtgraph
                            self.IXC_plots[xtrace].plot(
                                self.lags, self.IXC_corr[xtrace]
                            )
                            self.IXC_plots[xtrace].plot(
                                [0, 0], [-0.5, 1.5], pen=pg.mkPen("gray", width=0.5)
                            )
                            self.IXC_plots[xtrace].plot(
                                [np.min(self.lags), np.max(self.lags)],
                                [0, 0],
                            ...
                            if xtrace == 0:
                                self.IXC_plots[0].registerPlot(
                                    name="xcorr_%03d" % xtrace
                                )
                            if xtrace > 0:
                                self.IXC_plots[xtrace].vb.setXLink(
                                    "xcorr_000"
                                )  # not sure - this seems to be at the wrong level in the window manager
                                self.IXC_plots[xtrace].vb.setYLink(
                                    "xcorr_000"
                                )  # not sure - this seems to be at the wrong level in the window manager
                        else:  # pylab
                    ...
                    if dlg.wasCanceled():
                        raise HelpfulException(
                            "Calculation canceled by user.", msgType="status"
                        )
        ...
        if not self.use_MPL and plottype == "traces":
            ...
            for i in range(0, xtrace):
                self.IXC_plots[i].setYRange(
                    ymin, ymax
                )  # remember, all are linked to the 0'th plot
    ...
    def Analysis_smcAnalyze(self):
        ...
        for roi in range(0, self.nROI):
            print("ROI: %d" % (roi))
            # normalized the data:
            ndat = (self.BFData[roi, :] - np.min(self.BFData[roi, :])) / np.max(
                self.BFData[roi, :]
            )
            self.smc_V = SMC.Variables(ndat, dt)
            self.smc_P = SMC.Parameters(
                self.smc_V,
                A=self.smc_A,
                k_d=self.smc_Kd,
                C_0=self.smc_C0,
    ...
    def smc_AnalyzeMatlab(self):
        ...
        subprocess.call(
            ["/Applications/MATLAB_R2010b.app/bin/matlab", "-r", "FigSimNoisy.m"],
            bufsize=1,
        )
    ...
    def RegisterStack2(self):
        ...
        """
        try:
            from acq4.analysis.tools import (
                ImageP,
            )  # avaialable as part of the STXMPy package
        except:
    ...
    def readFromDb(self, sequenceDir=None, sourceFile=None):
        ...
        if sourceFile is not None:
            events = db.select(
                table, "*", where={"SourceFile": sourceFile}, toArray=True
            )
        else:
            events = db.select(
                table, "*", where={"ProtocolSequenceDir": sequenceDir}, toArray=True
            )
        ...
        if events is None:
            ...
            events = np.empty(
                0,
                dtype={"names": [k for k in schema], "formats": [object] * len(schema)},
            )
